...
///        and as multigrid smoothers on the EpsilonDivDiv (viscous) operator.
///
...
///   - Naked smoother comparison (point vs block Jacobi)
///   - Multigrid V-cycle comparison (point vs block Jacobi as smoother)
...
std::pair< double, double > run_smoother_comparison(
...
/// @brief Interpolate coefficient field on a given level's domain.
void interpolate_k_lateral(
    DistributedDomain&                domain,
    const Grid3DDataVec< double, 3 >& coords,
    const Grid2DDataScalar< double >& radii,
    ...
    double                            contrast )
{
    Kokkos::parallel_for(
        "k_lateral",
        grid::shell::local_domain_md_range_policy_nodes( domain ),
        LateralViscosityInterpolator{ coords, radii, k_data, contrast } );
    Kokkos::fence();
}

/// @brief Build a V-cycle multigrid with the given smoother type and run it.
/// @tparam SmootherSetupFn Callable (Viscous&, domain, mask, inv_diag, omega, tmp) -> Smoother
...
std::pair< double, double > run_multigrid_comparison(
    const std::string&                                                          label,
    const int                                                                   min_level,
    const int                                                                   max_level,
    ...
    const int                                                                   num_cycles )
{
    ...
    // Build operators (one per level) with the appropriate coefficient field.
    ...
    for ( size_t level = 0; level < num_levels; ++level )
    {
        // Compute point inverse diagonal.
        ...
        // Compute omega via power iteration.
    ...
    // Compute final residual for point.
    ...
    std::cout << "MG Final:  point=" << final_r_point << "  block=" << final_r_block
              << "  ratio(block/point)=" << final_r_block / final_r_point << std::endl;

    return { final_r_point, final_r_block };
}
...
int main( int argc, char** argv )
{
    ...
    std::cout << "\n################################################################" << std::endl;
    std::cout << "# Part 1: Naked smoother comparison" << std::endl;
    std::cout << "################################################################" << std::endl;
    ...
    std::cout << "\n################################################################" << std::endl;
    std::cout << "# Part 2: Multigrid V-cycle comparison (point vs block Jacobi)" << std::endl;
    std::cout << "################################################################" << std::endl;
