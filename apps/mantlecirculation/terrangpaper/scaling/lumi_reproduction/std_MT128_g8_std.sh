#!/bin/bash -l
#SBATCH --job-name=ls_MT128_g8_std
#SBATCH --output=/scratch/project_465002367/bohmfabi/scal_a3_lumi_std/logs/MT128_g8_std.o%j
#SBATCH --error=/scratch/project_465002367/bohmfabi/scal_a3_lumi_std/logs/MT128_g8_std.e%j
...
echo "Cell: MT128_g8_std  mesh=[2..7]  lat_sdr=1 rad_sdr=0  steps=10  fgmres=10  ev=50  n_gcds=8  nodes=1x8"
...
OUT="${TERRANG_OUT:-/scratch/project_465002367/bohmfabi/scal_a3_lumi_std/MT128_g8_std}"
...
srun --cpu-bind=map_cpu:49,57,17,25,1,9,33,41 ${SELECT_GPU} "$BIN" --config "$CFG" --extended-parameters \
  ...
  --lat-sdr 1 --rad-sdr 0 --radial-extra-levels -1 \
