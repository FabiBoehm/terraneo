#!/bin/bash -l
#SBATCH --job-name=ls_MT1024_g2048_std
#SBATCH --output=/scratch/project_465002367/bohmfabi/scal_a3_lumi_std/logs/MT1024_g2048_std.o%j
#SBATCH --error=/scratch/project_465002367/bohmfabi/scal_a3_lumi_std/logs/MT1024_g2048_std.e%j
...
echo "Cell: MT1024_g2048_std  mesh=[5..10]  lat_sdr=5 rad_sdr=0  steps=10  fgmres=10  ev=50  n_gcds=2048  nodes=256x8"
...
OUT="${TERRANG_OUT:-/scratch/project_465002367/bohmfabi/scal_a3_lumi_std/MT1024_g2048_std}"
...
srun --cpu-bind=map_cpu:49,57,17,25,1,9,33,41 ${SELECT_GPU} "$BIN" --config "$CFG" --extended-parameters \
  ...
  --refinement-level-mesh-min 5 --refinement-level-mesh-max 10 \
