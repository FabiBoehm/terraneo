#!/bin/bash -l
#SBATCH --job-name=lr_MT128_g256_lowmem
#SBATCH --output=/scratch/project_465002367/bohmfabi/scal_a3_lumi/logs/MT128_g256_lowmem.o%j
#SBATCH --error=/scratch/project_465002367/bohmfabi/scal_a3_lumi/logs/MT128_g256_lowmem.e%j
...
echo "Cell: MT128_g256_lowmem  mesh=[4..7]  subdomains-level=3  steps=10  fgmres=10  ev=50  n_gcds=256  nodes=32x8"
...
OUT="${TERRANG_OUT:-/scratch/project_465002367/bohmfabi/scal_a3_lumi/MT128_g256_lowmem}"
...
srun --cpu-bind=map_cpu:49,57,17,25,1,9,33,41 ${SELECT_GPU} "$BIN" --config "$CFG" --extended-parameters \
  ...
  --refinement-level-mesh-min 4 --refinement-level-mesh-max 7 \
