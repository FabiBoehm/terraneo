#!/bin/bash -l
#SBATCH --job-name=ls_MT2048_g1024_std
#SBATCH --output=/scratch/project_465002367/bohmfabi/scal_a3_lumi_std/logs/MT2048_g1024_std.o%j
#SBATCH --error=/scratch/project_465002367/bohmfabi/scal_a3_lumi_std/logs/MT2048_g1024_std.e%j
...
echo "Cell: MT2048_g1024_std  mesh=[4..11]  lat_sdr=4 rad_sdr=1  steps=10  fgmres=10  ev=50  n_gcds=1024  nodes=128x8"
...
OUT="${TERRANG_OUT:-/scratch/project_465002367/bohmfabi/scal_a3_lumi_std/MT2048_g1024_std}"
...
srun --cpu-bind=map_cpu:49,57,17,25,1,9,33,41 ${SELECT_GPU} "$BIN" --config "$CFG" --extended-parameters \
  ...
  --refinement-level-mesh-min 4 --refinement-level-mesh-max 11 \
  --lat-sdr 4 --rad-sdr 1 --radial-extra-levels -1 \
