/**
 * Measures attained memory bandwidth on NVIDIA GPUs comparing:
 *   - Raw CUDA (cudaMalloc + __global__ kernel)
 ...
 *
 * CUDA counterpart of benchmark_bandwidth.cpp (HIP / MI250X).
 */
...
Result bench_raw( size_t N, int warmup, int iterations, double scalar )
{
    ...
    CUDA_CHECK( cudaMemset( d_C, 2, bytes ) );
    CUDA_CHECK( cudaDeviceSynchronize() );

    for ( int w = 0; w < warmup; ++w )
        triad_raw<<< grid, block_size >>>( d_A, d_B, d_C, scalar, N );
    CUDA_CHECK( cudaDeviceSynchronize() );
...
int main( int argc, char* argv[] )
{
    ...
    std::cerr << "size_bytes,size_mb,raw_cuda,kokkos_1d,kokkos_2d,kokkos_2d_static,kokkos_3d,kokkos_3d_static,kokkos_4d,kokkos_4d_static\n";
