...
for name, root, LM, KM in CASES:
    ...
    net = LinearOperator(5, 4, Sv[1:4], coords, n_hidden=ck["hidden"],
                         ...
                         eta_stencils=ck.get("linear_eta_stencils", 0), multi_dilation=ck.get("linear_multi_dilation", False), mode_attn=ck.get("linear_mode_attn", 0), eta_embed_dim=ck.get("linear_eta_embed_dim", 16), eta_quant=ck.get("linear_eta_quant", 0), seam_average=True, bank_bottleneck=ck.get("linear_bank_bottleneck", 0), sep_stencils=ck.get("linear_sep_stencils", False), phys_attn=ck.get("linear_phys_attn", 0), phys_attn_dim=ck.get("linear_phys_attn_dim", 32), phys_attn_layers=ck.get("linear_phys_attn_layers", 1), spectral_layers=ck.get("linear_spectral_layers", 1), spectral=ck.get("linear_spectral", True), phys_window_physical=ck.get("linear_phys_window_physical", False), phys_grad_feat=ck.get("linear_phys_grad_feat", False), phys_mode=ck.get("linear_phys_mode", "mlp"), green_patch_cond=ck.get("linear_green_patch_cond", False),
                         green_mlp=True).eval()
    lev = int(mesh["level"])
    ...
    for ck_i in cks:
        net_i = LinearOperator(5, 4, Sv[1:4], coords, n_hidden=ck_i["hidden"], n_blocks=ck_i["heads"], lmax=LM, kmax=KM,
                               ...
                               eta_stencils=ck_i.get("linear_eta_stencils", 0), multi_dilation=ck_i.get("linear_multi_dilation", False), mode_attn=ck_i.get("linear_mode_attn", 0), eta_embed_dim=ck_i.get("linear_eta_embed_dim", 16), eta_quant=ck_i.get("linear_eta_quant", 0), seam_average=True, bank_bottleneck=ck_i.get("linear_bank_bottleneck", 0), sep_stencils=ck_i.get("linear_sep_stencils", False), phys_attn=ck_i.get("linear_phys_attn", 0), phys_attn_dim=ck_i.get("linear_phys_attn_dim", 32), phys_attn_layers=ck_i.get("linear_phys_attn_layers", 1), spectral_layers=ck_i.get("linear_spectral_layers", 1), spectral=ck_i.get("linear_spectral", True), phys_window_physical=ck_i.get("linear_phys_window_physical", False), phys_grad_feat=ck_i.get("linear_phys_grad_feat", False), phys_mode=ck_i.get("linear_phys_mode", "mlp"), green_patch_cond=ck_i.get("linear_green_patch_cond", False),
                               green_mlp=True).eval()
